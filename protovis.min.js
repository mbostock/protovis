...
a.context=function(b,c,d){function f(q,n){pv.Mark.scene=q;h.index=n;if(q){var p=q.mark,m=p,r=[];do{r.push(m);i.push(q[n].data);m.index=n;m.scene=q;n=q.parentIndex;q=q.parent}while(m=m.parent);q=r.length-1;for(n=1;q>0;q--){m=r[q];m.scale=n;n*=m.scene[m.index].transform.k}if(p.children){q=0;for(r=p.children.length;q<r;q++){m=p.children[q];m.scene=p.scene[p.index].children[q];m.scale=n}}}}function g(q){if(q){q=q.mark;var n;if(q.children)for(var p=0,m=q.children.length;p<m;p++){n=q.children[p];delete n.scene;
delete n.scale}n=q;do{i.pop();if(n.parent){delete n.scene;delete n.scale}delete n.index}while(n=n.parent)}}var h=pv.Mark.prototype,i=pv.Mark.stack,j=pv.Mark.scene,k=h.index;g(j,k);f(b,c);try{d.apply(this,i)}catch(l){pv.error(l)}finally{g(b,c);f(j,k)}};pv.Mark.dispatch=function(b,c,d){var f=c.mark,g=c.parent,h=f.$handlers[b];if(!h)return g&&pv.Mark.dispatch(b,g,c.parentIndex);f.context(c,d,function(){(f=h.apply(f,pv.Mark.stack))&&f.render&&f.render()});return true};
