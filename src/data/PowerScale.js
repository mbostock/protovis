/**
 * Returns a power scale for the specified domain. The arguments to this
 * constructor are optional, and equivalent to calling {@link #domain}.
 *
 * @class Represents a power scale. <style
 * type="text/css">sub{line-height:0}</style> <img src="../power.png"
 * width="190" height="175" align="right"> Most commonly, a power scale
 * represents a 1-dimensional exponential transformation from a numeric domain
 * of input data [<i>d<sub>0</sub></i>, <i>d<sub>1</sub></i>] to a numeric range
 * of pixels [<i>r<sub>0</sub></i>, <i>r<sub>1</sub></i>]. The equation for such
 * a scale is:
 *
 * <blockquote><i>f(x) = (pow(x) - pow(d<sub>0</sub>)) / (pow(d<sub>1</sub>) -
 * pow(d<sub>0</sub>)) * (r<sub>1</sub> - r<sub>0</sub>) +
 * r<sub>0</sub></i></blockquote>
 *
 * where <i>pow(x)</i> represents the zero-symmetric power of <i>x</i> using the
 * scale's associated exponent. For example, a power scale from the domain [1,
 * 100] to range [0, 640]:
 *
 * <blockquote><i>f(x) = (pow(x) - pow(1)) / (pow(100) - pow(1)) * (640 - 0) + 0</i><br>
 * <i>f(x) = pow(x) / 2 * 640</i><br>
 * <i>f(x) = pow(x) * 320</i><br>
 * </blockquote>
 *
 * Thus, saying
 *
 * <pre>.height(function(d) Math.pow(d) * 138.974)</pre>
 *
 * is equivalent to
 *
 * <pre>.height(pv.Scale.pow(1, 100).range(0, 640))</pre>
 *
 ...
 * example above to
 *
 * <pre>.fillStyle(pv.Scale.pow(1, 100).range("red", "green"))</pre>
 *
 ...
 *
 * <p>2. The domain and range can be subdivided for a "poly-pow"
 ...
 *
 * <pre>.fillStyle(pv.Scale.pow(1, 10, 100).range("red", "white", "green"))</pre>
 *
 ...
 * <i>n - 1</i> contiguous pow scales.
 *
 * <p>3. Pow scales can be inverted for interaction. The {@link #invert} method
 ...
 * @returns {pv.Scale.pow} a power scale.
 */
pv.Scale.power = function() {
  var d = [0, 1], l = [0, 1], b = 1, r = [0, 1], i = [pv.identity];
  ...
  function scale(x) {
    ...
    return i[j]((pow(x) - l[j]) / (l[j + 1] - l[j]));
  }
   ...
   * poly-power scales, multiple values can be specified. Values can be derived
   ...
   *
   * @function
   * @name pv.Scale.power.prototype.domain
   * @param {number...} domain... domain values.
   * @returns {pv.Scale.power} <tt>this</tt>, or the current domain.
   */
  ...
  /**
   * @function
   * @name pv.Scale.power.prototype.range
   * @param {...} range... range values.
   * @returns {pv.Scale.power} <tt>this</tt>.
   */
...
//    * equivalent strings. For a diverging scale, or other subdivided poly-power
...
//    *
//    * @function
//    * @name pv.Scale.power.prototype.invert
//    * @param {...} range... range values.
//    * @returns {pv.Scale.power} <tt>this</tt>, or the current range.
//    */
...
//    *
//    * @function
//    * @name pv.Scale.power.prototype.ticks
...
//    *
//    * @function
//    * @name pv.Scale.power.prototype.nice
//    * @returns {pv.Scale.power} <tt>this</tt>.
//    */
//   scale.nice = function() {
//     // TODO: support multiple domains
...
//   };
  ...
  /**
   * Sets or gets the exponent. Defaults to 1.
   *
   * @function
   * @name pv.Scale.power.prototype.base
   * @param {number} [v] the new base.
   * @returns {pv.Scale.power} <tt>this</tt>, or the current base.
   */
  scale.exponent = function(v) {
    if (arguments.length) {
      b = v;
      l = d.map(pow);
      return this;
   ...
   *
   * <pre>.height(pv.Scale.power().range(0, 480).by(function(d) d.score))</pre>
   *
   ...
   *
   * @function
   * @name pv.Scale.power.prototype.by
   * @param {function} f an accessor function.
   * @returns {pv.Scale.power} a view of this scale by the specified accessor
   * function.
   */
  scale.by = function(f) {
...
pv.Scale.sqrt = function() {
  var scale = pv.Scale.power().exponent(1/2);
  scale.domain.apply(scale, arguments);
  return scale;
};
